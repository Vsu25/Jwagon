  ...
  try {
     switch (payload.action) {
         ...
         case 'countdown_reset':
             ...
             break;
         case 'toggle':
             // Toggle overlay element visibility via state manager
         ...
         case 'goals_save':
             ...
             break;
         case 'mock_event':
             // Simulate a Kick event for testing
             const eventType = payload.eventType || 'channel.subscription.new';
             const mockPayload = getMockPayload(eventType);
             eventRouter.handleEvent(userId, eventType, mockPayload);
             break;
         case 'set_position':
             // Set position for an overlay element
