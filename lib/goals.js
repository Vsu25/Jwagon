...
async function setGoals(userId, goalsList) {
    ...
    const { broadcast } = require('../server');
    broadcast({ type: 'goal:list', userId, data: { goals: goalsList } }, [userId]);
}
...
async function revertMocks(userId) {
  const mockAmount = mockTracking.get(userId) || 0;
  if (mockAmount === 0) return 0;
  
  const userState = await initUser(userId);
  const old = userState.currentCount;
  userState.currentCount = Math.max(0, userState.currentCount - mockAmount);
